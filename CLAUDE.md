...
- **Pod controller-owner labels (D34).** Each `type="pod"` node carries `labels.owner_kind` + `labels.owner_name` for its controller owner, resolved from `kube_pod_owner` with the **ReplicaSet skipped to its owning Deployment** via `kube_replicaset_owner` (a bare ReplicaSet with no Deployment owner stays as-is; other owner kinds surface verbatim). Both keys are **omitted** when the pod has no controller owner — never empty strings. Both series are KSM defaults (no `--metric-labels-allowlist`) and OPTIONAL (absence degrades gracefully, no build failure). Resolution lives in `pkg/build/topology.go` (`resolvePodOwners`); the controller pick is deterministic (lexically-smallest `(kind, name)` on collision). No new node/edge type — strict `map[string]string` labels only.
