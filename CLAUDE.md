...
- **Request surface is `start`, `end`, `cluster`, `namespace`, `az`, `env`, `edge_type`, `prune`** on `/v1/graph` — everything but `start` / `end` optional. `GET /v1/storage-graph` additionally **requires** single-valued `az` and `env` (`missing_az` / `missing_env`; a repeat is `invalid_scope`) and accepts optional repeatable roots `ontap_cluster`, `aggr`, `svm`, `pod=<ns>/<name>`, `node` (matched against both ONTAP controller and Kubernetes node names); `edge_type` / `prune` are ignored there. `name`, `root`, `depth` and `direction` are **withdrawn** (BREAKING) and, like any unknown parameter, ignored without error: an old client receives the unanchored view. `GET /v1/clusters` is **removed** (BREAKING) together with the `cluster_discovery` query — the cluster list is the `clusters` field of any `/v1/graph` response. `graph.Scope` is `{Clusters, Namespaces, EdgeTypes, Inventory}`; `traverse` / `MaxTraversalDepth` / `Direction` / `Names` are gone. `graph.StorageScope` is `{Clusters, Namespaces, Roots}`.
...
- **NetApp storage join is three independently-degrading hops** (`pkg/build/netapp.go`
  ...
  **stock** Harvest `volume` label (the ONTAP FlexVol name). It is NOT a label
  ...
  list of regex rewrite rules plus a match mode, both operator-configurable
  (`--netapp-volume-key-rewrite`, `--netapp-volume-match-mode`), defaulting to
  "replace `-` with `_`, match as a **suffix**" — which resolves a stock Trident
  estate without the deployment declaring its `storagePrefix`, while rejecting a
  clone whose name extends past the PV name. `pkg/build/volumekey.go` owns it;
  `exact` / `suffix` resolve through a length-bucketed hash index (O(volumes)),
  `contains` / `regex` scan. An uncompilable pattern or unknown mode is a
  STARTUP failure (`internal/config`), never a silent fallback. **No relabel
  rule is required or read.** Hops:
