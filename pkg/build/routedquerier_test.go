...
func TestReadTopology_RoutedFanOutReachesEveryBackend(t *testing.T) {
	...
	// Task 5.4: the 43-leg fan-out is per BUILD, not per backend.
	assert.Equal(t, 43, fa.legCount(), "each backend sees the full leg set")
	assert.Equal(t, 43, fb.legCount())
	assert.Equal(t, 43, fa.totalCalls(), "each leg issued exactly once per backend")
	assert.Equal(t, 43, fb.totalCalls())
}
