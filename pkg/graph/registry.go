...
var EdgeTypes = []EdgeTypeDefinition{
	...
	},
	{
		...
		MayCrossCluster: false,
		Labels: []EdgeTypeLabel{
			{Name: "tier", ValueType: "string", Description: "The hop this edge covers on the fixed chain: one of node-aggr, aggr-svm, svm-pvc, pvc-pod, pod-node."},
			{Name: "attribution", ValueType: "string", Description: "Set to 'split' on a pvc-pod edge whose claim is mounted by more than one pod: per-pod I/O is not observable, so the claim's weight is divided equally across its mounters. Absent on every other edge, including a singly-mounted claim's."},
		},
	},
}
