...
- `type` SHALL be one of the strings `"pod"`, `"node"`, `"pvc"`, `"service"`, `"external"`. The Cytoscape serialiser additionally synthesises `"cluster"` group nodes for compound nesting (see "Cytoscape compound node grouping").
