...
```json
{
  ...
  "edge_types": [
    ...
    {
      "type": "pod-calls-pod",
      "description": "Pod-UID-resolved RPC edge from service-graph metrics. May cross clusters when the resolved source and target pods live in different clusters (recovered from the topology pod-UID index since the metric only carries the trace-source cluster). Endpoints may be 'service' nodes when a '://' connection-string label resolves in topology (D29; the headless per-pod form resolves to the same service, not a specific pod), 'others' nodes when a '://' label does not resolve (D29), or 'external' nodes via the missing-UID human-label fallback (D27).",
...
1. **Connection-string resolution** (D29; only when UID is empty AND the human label contains `"://"`): resolves to a `service` node (plus on-demand `service-selects-pod` fan-out edges) or — on miss — an `others` node (`id=others/<label>`, `labels={}`). Never a pod.
...
1. Connection-string resolution (hardcoded `"://"`; only when UID empty AND label contains `"://"`) → service node (+ `service-selects-pod` edges) OR (on miss) `others/<label>` with `labels={}`. Never a pod.
...
A `"://"`-bearing label with an empty UID therefore always flows through Stage 1 and never reaches the external fallback. `others` and `external` remain **disjoint** by node `type` and ID namespace (`others/<label>` vs `external/<label>`): `others` now means "a recognised `"://"` connection string that did not resolve to an in-cluster service" (a declared external dependency); `external` is reserved for missing-UID endpoints with a **non-URL** human label (the producer-regression signal). The `pod-calls-pod` edge `labels.cluster` rule (D9) is unchanged — present only when the **client** side resolves to a pod. A client `"://"` label always resolves to a service or others node (never a pod), so such an edge always **omits** `cluster`, consistent with every other non-pod client side. Service nodes are not pods.
...
- Recognising an in-cluster Service named by `<service>.<namespace>.svc.<domain>` lets the graph show the real Service → backing-pod fan-out instead of collapsing every in-cluster DB / cache / API call into an opaque `others` blob. Headless `<pod>.<service>.<namespace>` records (StatefulSets) resolve to the **same** Service node and fan out to all of its backing pods — one uniform shape for every in-cluster `"://"` reference, not a separate per-pod path.
