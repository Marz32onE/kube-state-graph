...
**Discovery：** `GET /v1/clusters` 由 `group by (cluster) (last_over_time(kube_node_info[1h]))` 即時導出，lookback 固定寫死為 `1h`（足以吸收暫時性 KSM scrape gap，不開放配置）。每次請求都打 VictoriaMetrics，v1 不附帶 in-process discovery cache，呼叫端透過 `ETag` revalidate。
