...
3. 將 view 序列成請求的 `format`（Cytoscape.js 或 Grafana Node Graph）並回應。
...
**Response cache headers。** 內容穩定的 route 明示 `Cache-Control`（`/v1/edge-types`：3600 s、`/openapi.{yaml,json}`：3600 s、`/docs/assets/*`：86400 s、`/docs`：300 s）以利 browser 與 reverse proxy 之 client-side cache。`/v1/graph`、`/v1/graph/nodegraph`、`/v1/clusters` 不發 `Cache-Control`——每次 fresh build 之 body 視為權威。
...
Service-graph metrics 帶 Tempo 風格 `client` / `server` 人類可讀 label。預設以 `(cluster, client_k8s_pod_uid)` 解析 client 端 pod；server 端則以 `server_k8s_pod_uid` lookup 全域 topology pod-UID index 還原。非 pod 遠端（外部 HTTP、託管 DB、queue、SaaS）可經環境變數 `KSG_OTHERS_NAME_PATTERN`（或 flag `--others-name-pattern`）以 **substring contains** 判斷：若 label 值包含 pattern，則該端為 `others` node（`id = "others/<label_value>"`、`type="others"`、`labels.pattern=<pattern>`）。Client/server 兩側獨立；edge `type` 仍為 `pod-calls-pod`。Edge 之 `labels.cluster` 僅當 **client** 端為 pod 時才設值；client 端為非 pod（`others` 或 D27 fallback 之 `external`）時，edge `labels` map 中省略 `cluster` key（非 pod 端無 cluster scope）。

空 pattern 則關閉規則，回到純 pod UID 解析。

`others/<label>` 與 D27 missing-UID fallback 產生的 `external/<label>` 為 **disjoint** 命名空間與 dedupe map——同一 label 字串被兩種規則命中時會產生兩個不同節點（intentional：operator 宣告的第三方端點 vs producer 退化的推論端點屬不同營運語意；混用會掩蓋 producer regression 的訊號）。
...
- `KSG_OTHERS_NAME_PATTERN` 是否演進為 regex（`KSG_OTHERS_NAME_REGEX`）或接受多個逗號分隔 pattern——依真實部署回饋延到 v1.x。
- Others node 是否應暴露額外 `labels`（例如從 URL 形狀值解析出 scheme）——延後；v1 僅 `labels.pattern`。
