...
| Node | `name` | string | Pod 名 / node 名 / PVC claim 名 / service 名。Others / external node 為 `client` 或 `server` label 原文（連線字串）。Grafana panel 顯示用。 |
...
| Edge | `type` | string | `/v1/edge-types` 註冊型別之一（如 `"pod-runs-on-node"`、`"pod-mounts-pvc"`、`"pod-calls-pod"`、`"service-selects-pod"`）。 |
| Edge | `source` / `target` | string | 同回應內存在之 node `id`。 |
| Edge | `labels` | `map[string]string` | `pod-calls-pod`：`cluster`（trace 來源／client 端 cluster；client 端為非 pod 端點—`service`、`others` 或 `external`—時省略）。`pod-mounts-pvc`：`claim_name`、`storage_class`。`pod-runs-on-node`：`scheduled_at`。`service-selects-pod`：可選 `namespace`（無必填 key）。新 key additive。 |
...
**(11) 本機 rig / KSM。** 於 KSM `--resources` 與 `--metric-allowlist` 啟用 services + endpointslices；擴充 RBAC（list/watch services、endpointslices）；自 rig Deployment 與 Helm chart **移除** `KSG_OTHERS_NAME_PATTERN` env。（由 `tasks.md` 追蹤。）
