...
- `graph-api`：HTTP API 表面（Gin），回傳合併後的跨 cluster pod / node / PVC / service graph（Cytoscape.js JSON）、Grafana Node Graph 相容 route、時間區間參數、filtering、partial-graph traversal、edge-type discovery、cluster discovery。node 類型集合包含 `type="service"` node（ClusterIP／service 層級連線字串的目標），edge-type catalogue 包含 `service-selects-pod` edge（有向 service→pod、intra-cluster）。
